...
function fileTypeFromPath(filePath, rootPath) {
  const lower = path6.relative(rootPath, filePath).toLowerCase().replace(/\\/g, "/");
  if (lower.startsWith("knowledge") || lower.includes("/knowledge/")) return "knowledge";
  if (lower.startsWith("projects") || lower.includes("/projects/")) return "project";
  if (lower.startsWith("tasks") || lower.includes("/tasks/")) return "task";
  if (lower.startsWith("daily") || lower.startsWith("02 harian") || lower.includes("/02 harian/"))
...
function realPathForTarget(resolved) {
  if (fs7.existsSync(resolved)) return fs7.realpathSync.native(resolved);
  let current = path9.dirname(resolved);
  while (!fs7.existsSync(current)) {
    const parent = path9.dirname(current);
    if (parent === current) return resolved;
    current = parent;
  }
  return path9.join(fs7.realpathSync.native(current), path9.relative(current, resolved));
}
function assertPathInVault(filePath, vaultRoot) {
  ...
  const root = path9.resolve(vaultRoot);
  const resolved = path9.resolve(filePath);
  assertLexicallyInVault(resolved, root);
  const realRoot = fs7.existsSync(root) ? fs7.realpathSync.native(root) : root;
...
function resolveNotePath(input) {
  ...
  if (direct && fs8.existsSync(direct)) return direct;
  if (direct && !fs8.existsSync(direct) && direct.toLowerCase().endsWith(".md")) {
    return direct;
  }
...
function relativeToVault(abs) {
  const root = workspaceEngine.getState().rootPath || "";
  return path10.relative(root, abs).replace(/\\/g, "/");
}
  ...
  try {
    ...
    switch (tool) {
      ...
      case "list_dir": {
        ...
        const entries = fs8.readdirSync(abs, { withFileTypes: true });
        const items = entries.filter((e) => !e.name.startsWith(".")).slice(0, 100).map((e) => ({
          name: e.name,
          type: e.isDirectory() ? "dir" : "file",
          path: relativeToVault(path10.join(abs, e.name))
        }));
      ...
      case "write_note": {
        ...
        if (fs8.existsSync(abs) && !content.startsWith("---")) {
          try {
      ...
      case "create_note": {
        ...
        if (!content.startsWith("---")) {
          const title = path10.basename(abs, ".md") || "Untitled";
      ...
      case "create_from_template": {
        ...
        const content = templateEngine.render(tpl.body, {
          ...
          workspace: path10.basename(root)
        });
        ...
        const abs = resolvePath(path10.join(folder, fileName));
        if (!abs) return { tool, ok: false, error: "Invalid path" };
        if (fs8.existsSync(abs)) {
          return { tool, ok: false, error: `File already exists: ${relativeToVault(abs)}` };
        }
...
var AIMiddleware = class {
  ...
  async testProvider(providerId) {
    ...
    if (!provider.isConfigured()) {
      return {
        ok: false,
        error: id === "grok" ? "Grok belum dikonfigurasi. Settings \u2192 Import Grok CLI, atau paste XAI_API_KEY." : "API key / base URL belum dikonfigurasi"
      };
    }
    try {
