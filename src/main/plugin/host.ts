...
export class PluginHost {
  ...
  async loadPlugin(manifestPath: string): Promise<any> {
    ...
    this.plugins.set(manifest.id, { manifest, exports: pluginModule, api: api, disable: () => this.unload(manifest.id), dir: join(manifestPath, '..') })
    return { manifest, exports: pluginModule, api: api, disable: () => this.unload(manifest.id), dir: join(manifestPath, '..') }
  }
...
return {
      ...
      workspace: createNamespace(['getActiveFile', 'getOpenFiles', 'openFile', 'newFile', 'onActiveFileChange']),
      editor: createNamespace(['getContent', 'setContent', 'getSelection', 'replaceSelection', 'insertAtCursor', 'getCursor', 'setCursor']),
      graph: createNamespace(['getNodes', 'getEdges', 'getNeighbors', 'getBacklinks', 'getOrphans']),
      ...
      ui: createNamespace(['notify', 'showInput', 'showConfirm', 'showSelect', 'showProgress', 'hideProgress', 'openPanel', 'closePanel']),
    }
  }
