...
### M4b: Minimal viable management per sistem — ✅ 6/6 selesai
...
| 3 | DOM-3 (daily) | Daily | Timeline harian sederhana (daftar harian kronologis + filter minggu/bulan) | ✅ 5 terbaru, filter daily type/path |
...
> **Status M5: ✅ SELESAI — 8/8 + polish penuh. UI-1 semua text font-size→token (icon-size tetap px), hex→token, zIndex 9999→--z-dropdown; UI-9/10/14/15/29 + palette CSS-var selesai.**

### M5a: Design tokens & styling — ✅

| # | Audit ID | Masalah | Solusi | Status |
|---|---|---|---|---|
| 1 | UI-1 | ±100 font-size px mentah (<10px) + warna hex + z-index inline | Migrasi ke token; hapus zIndex 9999; ganti font <10px | ✅ 119 text font-size→token (8 <10px, 31 JSX, 112 CSS text; 7 icon-size dipertahankan px) + zIndex 9999→--z-dropdown + hex→--text-on-primary |
...
> **Status M7: ✅ SELESAI — 20/20 (semua kecuali polish low terserap). W2 async, G3 folder, S1, G5, M1/M2 semua selesai.**

### M7: Skala & kualitas engine (spec 05-09)

| # | Audit ID | Masalah | Solusi | Status |
|---|---|---|---|---|
| 1 | W2 | scanDirectory sinkron + statSync → UI freeze di 10k+ file | Async scan / batas waktu / background | ✅ `scanDirectoryAsync` + `refreshFilesAsync` (yield tiap 100) |
...
| # | Audit ID | Masalah | Solusi |
|---|---|---|---|
| 1 | TST-1 | Tidak ada E2E renderer | Playwright/Electron E2E: buka vault fixture -> buat note -> link wiki -> graph -> chat (mock provider) | ✅ app.e2e.spec.ts |
